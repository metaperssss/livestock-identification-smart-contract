...
import { $query, $update, type Record, StableBTreeMap, type Vec, match, Result, type nat64, ic, Opt } from 'azle'
...
globalThis.crypto = {
  // @ts-expect-error: Azle requires a workaround to support the uuid package, which uses the getRandomValues method not available in Node.js environments.
  getRandomValues: () => {
